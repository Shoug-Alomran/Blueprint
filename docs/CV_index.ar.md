---
title: قوالب السيرة الذاتية
role: كتالوج القوالب
scope: خيارات قوالب السيرة الذاتية وآلية الشراء
constraints: استضافة معاينة خاصة وتحكم في الوصول للملفات
metrics: وضوح القوالب ومعدل التحويل للشراء
last_updated: 2026-03-02
---

# قوالب السيرة الذاتية

اختر القالب، عاين الشكل، ثم أرسل طلب الشراء.

---

## ملخص الباقات

<div class="card-grid hero-cards">

  <article class="hero-card">
    <h3>Basic · 68 SAR</h3>
    <p>قالب سيرة بلغة واحدة بتصميم احترافي واضح مع جاهزية للطباعة PDF.</p>
  </article>

  <article class="hero-card">
    <h3>Pro · 95 SAR</h3>
    <p>قوالب متقدمة مع الوضع الليلي/النهاري، ودعم عربي/إنجليزي، وتجربة عرض أفضل.</p>
  </article>

  <article class="hero-card">
    <h3>التسليم</h3>
    <p>شراء القالب مباشرة أو تنفيذ وتكامل كامل داخل موقع Blueprint الحالي.</p>
  </article>

  <article class="hero-card">
    <h3>إعداد الخصوصية</h3>
    <p>يمكن إبقاء ملفات HTML خارج النشر العام وربط الكتالوج بروابط معاينة خاصة.</p>
  </article>

</div>

---

## قوالب Pro

<div class="cv-template-grid">

  <article class="info-card cv-template-card">
    <a class="cv-preview-shell" href="/cv-templates/temp_2_pro.html" target="_blank" rel="noopener">
      <iframe src="/cv-templates/temp_2_pro.html" loading="lazy" title="Template 2 preview"></iframe>
      <span class="cv-preview-label">فتح المعاينة</span>
    </a>
    <h3>Template 2 · Pro</h3>
    <p class="cv-template-theme">الثيم: Midnight Grid</p>
    <p>وضع ليلي/نهاري، تحويل عربي/إنجليزي، ودعم التنزيل والطباعة.</p>
    <p><a href="/cv-templates/temp_2_pro.html" target="_blank" rel="noopener">فتح المعاينة</a> · <a href="mailto:blueprint@shoug-tech.com?subject=CV Template Purchase — Template 2 (Pro)">شراء</a></p>
  </article>

  <article class="info-card cv-template-card">
    <a class="cv-preview-shell" href="/cv-templates/temp_3_pro.html" target="_blank" rel="noopener">
      <iframe src="/cv-templates/temp_3_pro.html" loading="lazy" title="Template 3 preview"></iframe>
      <span class="cv-preview-label">فتح المعاينة</span>
    </a>
    <h3>Template 3 · Pro</h3>
    <p class="cv-template-theme">الثيم: Editorial Sand</p>
    <p>وضع ليلي/نهاري، تحويل عربي/إنجليزي، ودعم التنزيل والطباعة.</p>
    <p><a href="/cv-templates/temp_3_pro.html" target="_blank" rel="noopener">فتح المعاينة</a> · <a href="mailto:blueprint@shoug-tech.com?subject=CV Template Purchase — Template 3 (Pro)">شراء</a></p>
  </article>

  <article class="info-card cv-template-card">
    <a class="cv-preview-shell" href="/cv-templates/temp_5_pro.html" target="_blank" rel="noopener">
      <iframe src="/cv-templates/temp_5_pro.html" loading="lazy" title="Template 5 preview"></iframe>
      <span class="cv-preview-label">فتح المعاينة</span>
    </a>
    <h3>Template 5 · Pro</h3>
    <p class="cv-template-theme">الثيم: Finance Prestige</p>
    <p>وضع ليلي/نهاري، تحويل عربي/إنجليزي، ودعم التنزيل والطباعة.</p>
    <p><a href="/cv-templates/temp_5_pro.html" target="_blank" rel="noopener">فتح المعاينة</a> · <a href="mailto:blueprint@shoug-tech.com?subject=CV Template Purchase — Template 5 (Pro)">شراء</a></p>
  </article>

  <article class="info-card cv-template-card">
    <a class="cv-preview-shell" href="/cv-templates/temp_6_pro.html" target="_blank" rel="noopener">
      <iframe src="/cv-templates/temp_6_pro.html" loading="lazy" title="Template 6 preview"></iframe>
      <span class="cv-preview-label">فتح المعاينة</span>
    </a>
    <h3>Template 6 · Pro</h3>
    <p class="cv-template-theme">الثيم: Monochrome Executive</p>
    <p>وضع ليلي/نهاري، تحويل عربي/إنجليزي، ودعم التنزيل والطباعة.</p>
    <p><a href="/cv-templates/temp_6_pro.html" target="_blank" rel="noopener">فتح المعاينة</a> · <a href="mailto:blueprint@shoug-tech.com?subject=CV Template Purchase — Template 6 (Pro)">شراء</a></p>
  </article>

  <article class="info-card cv-template-card">
    <a class="cv-preview-shell" href="/cv-templates/temp_7_pro.html" target="_blank" rel="noopener">
      <iframe src="/cv-templates/temp_7_pro.html" loading="lazy" title="Template 7 preview"></iframe>
      <span class="cv-preview-label">فتح المعاينة</span>
    </a>
    <h3>Template 7 · Pro</h3>
    <p class="cv-template-theme">الثيم: Deep Ocean</p>
    <p>وضع ليلي/نهاري، تحويل عربي/إنجليزي، ودعم التنزيل والطباعة.</p>
    <p><a href="/cv-templates/temp_7_pro.html" target="_blank" rel="noopener">فتح المعاينة</a> · <a href="mailto:blueprint@shoug-tech.com?subject=CV Template Purchase — Template 7 (Pro)">شراء</a></p>
  </article>

  <article class="info-card cv-template-card">
    <a class="cv-preview-shell" href="/cv-templates/temp_10_pro.html" target="_blank" rel="noopener">
      <iframe src="/cv-templates/temp_10_pro.html" loading="lazy" title="Template 10 preview"></iframe>
      <span class="cv-preview-label">فتح المعاينة</span>
    </a>
    <h3>Template 10 · Pro</h3>
    <p class="cv-template-theme">الثيم: Aurora Violet</p>
    <p>وضع ليلي/نهاري، تحويل عربي/إنجليزي، ودعم التنزيل والطباعة.</p>
    <p><a href="/cv-templates/temp_10_pro.html" target="_blank" rel="noopener">فتح المعاينة</a> · <a href="mailto:blueprint@shoug-tech.com?subject=CV Template Purchase — Template 10 (Pro)">شراء</a></p>
  </article>

</div>

---

## قوالب Basic

<div class="cv-template-grid">

  <article class="info-card cv-template-card">
    <a class="cv-preview-shell" href="/cv-templates/temp_1.html" target="_blank" rel="noopener">
      <iframe src="/cv-templates/temp_1.html" loading="lazy" title="Template 1 preview"></iframe>
      <span class="cv-preview-label">فتح المعاينة</span>
    </a>
    <h3>Template 1 · Basic</h3>
    <p class="cv-template-theme">الثيم: Clean Teal</p>
    <p>لغة واحدة، وضع نهاري، مع جاهزية كاملة للطباعة PDF.</p>
    <p><a href="/cv-templates/temp_1.html" target="_blank" rel="noopener">فتح المعاينة</a> · <a href="mailto:blueprint@shoug-tech.com?subject=CV Template Purchase — Template 1 (Basic)">شراء</a></p>
  </article>

  <article class="info-card cv-template-card">
    <a class="cv-preview-shell" href="/cv-templates/temp_4.html" target="_blank" rel="noopener">
      <iframe src="/cv-templates/temp_4.html" loading="lazy" title="Template 4 preview"></iframe>
      <span class="cv-preview-label">فتح المعاينة</span>
    </a>
    <h3>Template 4 · Basic</h3>
    <p class="cv-template-theme">الثيم: Classic Navy & Gold</p>
    <p>لغة واحدة، وضع نهاري، مع جاهزية كاملة للطباعة PDF.</p>
    <p><a href="/cv-templates/temp_4.html" target="_blank" rel="noopener">فتح المعاينة</a> · <a href="mailto:blueprint@shoug-tech.com?subject=CV Template Purchase — Template 4 (Basic)">شراء</a></p>
  </article>

  <article class="info-card cv-template-card">
    <a class="cv-preview-shell" href="/cv-templates/temp_8.html" target="_blank" rel="noopener">
      <iframe src="/cv-templates/temp_8.html" loading="lazy" title="Template 8 preview"></iframe>
      <span class="cv-preview-label">فتح المعاينة</span>
    </a>
    <h3>Template 8 · Basic</h3>
    <p class="cv-template-theme">الثيم: Professional Mono</p>
    <p>لغة واحدة، وضع نهاري، مع جاهزية كاملة للطباعة PDF.</p>
    <p><a href="/cv-templates/temp_8.html" target="_blank" rel="noopener">فتح المعاينة</a> · <a href="mailto:blueprint@shoug-tech.com?subject=CV Template Purchase — Template 8 (Basic)">شراء</a></p>
  </article>

  <article class="info-card cv-template-card">
    <a class="cv-preview-shell" href="/cv-templates/temp_9.html" target="_blank" rel="noopener">
      <iframe src="/cv-templates/temp_9.html" loading="lazy" title="Template 9 preview"></iframe>
      <span class="cv-preview-label">فتح المعاينة</span>
    </a>
    <h3>Template 9 · Basic</h3>
    <p class="cv-template-theme">الثيم: Minimal Ink</p>
    <p>لغة واحدة، وضع نهاري، مع جاهزية كاملة للطباعة PDF.</p>
    <p><a href="/cv-templates/temp_9.html" target="_blank" rel="noopener">فتح المعاينة</a> · <a href="mailto:blueprint@shoug-tech.com?subject=CV Template Purchase — Template 9 (Basic)">شراء</a></p>
  </article>

</div>

---

## ملاحظة الخصوصية

إذا قررت مستقبلًا استبعاد `cv-templates/**` من النشر العام، اربط هذه المسارات مع خادم معاينة خاص حتى تبقى روابط المعاينة شغالة بدون كشف الملفات المصدر.
