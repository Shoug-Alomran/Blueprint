(function () {
  ...
  function cleanHeadingText(node) {
    ...
    clone
      .querySelectorAll(".headerlink, .md-status, .md-nav__icon")
      .forEach(function (el) {
        el.remove();
      });
  ...
  function injectHeaderActions() {
    ...
    if (
      !right ||
      !titleWrap ||
      right.parentElement.querySelector(".header-actions")
    ) {
      return;
    }
    ...
    wrap.innerHTML = [
      '<a class="header-icon-btn header-github" href="https://github.com/Shoug-Alomran" target="_blank" rel="noopener" aria-label="' +
        labels.github +
        '">' +
        iconSvg(
          "M12 .5C5.65.5.5 5.66.5 12.02c0 5.08 3.29 9.38 7.86 10.9.58.11.79-.25.79-.56 0-.27-.01-1.01-.02-1.98-3.2.7-3.88-1.54-3.88-1.54-.53-1.35-1.28-1.71-1.28-1.71-1.04-.72.08-.71.08-.71 1.16.08 1.77 1.2 1.77 1.2 1.02 1.76 2.68 1.25 3.34.95.1-.75.4-1.25.72-1.54-2.55-.29-5.24-1.28-5.24-5.68 0-1.26.45-2.29 1.19-3.1-.12-.29-.52-1.46.11-3.03 0 0 .97-.31 3.17 1.18a10.9 10.9 0 0 1 5.78 0c2.2-1.49 3.17-1.18 3.17-1.18.63 1.57.23 2.74.11 3.03.74.81 1.19 1.84 1.19 3.1 0 4.41-2.69 5.39-5.26 5.67.41.36.77 1.06.77 2.14 0 1.55-.01 2.8-.01 3.18 0 .31.21.67.8.56A11.53 11.53 0 0 0 23.5 12C23.5 5.66 18.35.5 12 .5z",
        ) +
        "</a>",
      '<button class="header-icon-btn header-toggle-btn" data-sg-toggle="left" type="button" aria-label="' +
        labels.left +
        '">' +
        iconSvg("M3 5h6v14H3zm8 0h10v2H11zm0 6h10v2H11zm0 6h10v2H11z") +
        "</button>",
      '<button class="header-icon-btn header-toggle-btn" data-sg-toggle="right" type="button" aria-label="' +
        labels.right +
        '">' +
    ...
    function syncSidebarState() {
      ...
      if (!desktop) {
        document.body.classList.remove(
          "sg-hide-left-sidebar",
          "sg-hide-right-sidebar",
        );
        if (leftBtn) {
  ...
  function makeHeaderTitleGoHome() {
    ...
    titleWrap.setAttribute(
      "aria-label",
      isArabicPage() ? "العودة إلى الصفحة الرئيسية" : "Go to home page",
    );
  ...
  function enhanceMobileDrawerNav() {
    ...
    var primaryList = primaryNav
      ? primaryNav.querySelector(":scope > .md-nav__list")
      : null;
    ...
    primaryNav
      .querySelectorAll(".sg-mobile-quick-nav")
      .forEach(function (node) {
        node.remove();
      });
    primaryNav
      .querySelectorAll(".sg-mobile-section-context")
      .forEach(function (node) {
        node.remove();
      });
    ...
    quickNav.setAttribute(
      "aria-label",
      isArabicPage() ? "روابط أقسام الموقع" : "Site sections",
    );
    ...
    primaryList
      .querySelectorAll(":scope > .md-nav__item")
    ...
    var drawerTitle = primaryNav.querySelector(
      ":scope > .md-nav__title, :scope > label.md-nav__title",
    );
    var activeTopLevel = primaryList.querySelector(
      ":scope > .md-nav__item--active",
    );
    var insertionAnchor = drawerTitle;
    if (drawerTitle) {
      var activeParentLink = activeTopLevel
        ? activeTopLevel.querySelector(
            ":scope > .md-nav__container > a.md-nav__link, :scope > a.md-nav__link",
          )
        : null;
      var activePageNode = primaryNav.querySelector(
        ".md-nav__link--active .md-ellipsis",
      );
      var activePageLabel = activePageNode
        ? (activePageNode.textContent || "").trim()
        : "";
      var activeLabel = "";

      if (activeParentLink) {
        var activeLabelNode = activeParentLink.querySelector(".md-ellipsis");
        activeLabel = activeLabelNode
          ? (activeLabelNode.textContent || "").trim()
          : (activeParentLink.textContent || "").trim();
      } else {
        activeLabel = activePageLabel;
      }

      if (activeLabel) {
        var backHref = activeParentLink
          ? activeParentLink.getAttribute("href") || activeParentLink.href
          : "/";
        var context = document.createElement("div");
        context.className = "sg-mobile-section-context";
        context.innerHTML =
          '<a class="sg-mobile-section-context__back" href="' +
          backHref +
          '" aria-label="' +
          (isArabicPage() ? "الرجوع إلى القسم" : "Back to section") +
          '">' +
          iconSvg("M15.41 7.41 14 6l-6 6 6 6 1.41-1.41L10.83 12z") +
          "</a>" +
          '<div class="sg-mobile-section-context__body">' +
          '<span class="sg-mobile-section-context__eyebrow">' +
          (activePageLabel && activePageLabel !== activeLabel
            ? activePageLabel
            : isArabicPage()
              ? "القسم الحالي"
              : "Current section") +
          "</span>" +
          '<span class="sg-mobile-section-context__title">' +
          activeLabel +
          "</span>" +
          "</div>";
    ...
    primaryNav
      .querySelectorAll(
        "a.md-nav__link, .sg-mobile-quick-nav__link, .sg-mobile-section-context__back",
      )
  ...
  function buildTabDropdowns() {
    ...
    var primaryRoot = document.querySelector(
    ...
    primaryRoot
      .querySelectorAll(":scope > .md-nav__item")
      .forEach(function (item) {
        var parentLink = item.querySelector(
          ":scope > .md-nav__container > a.md-nav__link, :scope > a.md-nav__link",
        );
        if (!parentLink) {
          return;
        }
        var parentPath = normalizeNavPath(
          new URL(parentLink.href, window.location.origin).pathname,
        );
        var childLinks = [];

        item
          .querySelectorAll(":scope > .md-nav a.md-nav__link")
          .forEach(function (child) {
            var childPath = normalizeNavPath(
        ...
        if (
          parentPath === "/web-platforms/" &&
          uniqueChildren.some(function (entry) {
            return entry.path.indexOf("/work-demos/") === 0;
          })
        ) {
          dropdownItems.unshift({
            href: parentLink.getAttribute("href") || parentLink.href,
    ...
    tabs
      .querySelectorAll(":scope > .md-tabs__item")
      .forEach(function (tabItem) {
        ...
        var tabPath = normalizeNavPath(
          new URL(tabLink.href, window.location.origin).pathname,
        );
  ...
  function relabelProductsChildLinks() {
    ...
    document
      ...
      .forEach(function (link) {
        ...
        if (
          normalizeNavPath(new URL(href, window.location.origin).pathname) !==
          "/products/"
        ) {
          return;
        }
  ...
  function setupTabDropdownInteractions() {
    ...
    function positionOpenDropdowns() {
      tabs
        .querySelectorAll(".md-tabs__item--has-dropdown.is-open")
        .forEach(function (item) {
    ...
    tabs
      .querySelectorAll(".md-tabs__item--has-dropdown")
      .forEach(function (item) {
  ...
  function applyPageLayoutOverrides() {
    ...
    if (!shouldHideLeft && primaryRoot) {
      var activeTopLevel = primaryRoot.querySelector(
        ":scope > .md-nav__item--active",
      );
      if (activeTopLevel) {
        var parentLink = activeTopLevel.querySelector(
          ":scope > .md-nav__container > a.md-nav__link, :scope > a.md-nav__link",
        );
        var parentPath = "";
        ...
        activeTopLevel
          .querySelectorAll(
            ":scope > .md-nav > .md-nav__list > .md-nav__item > .md-nav__container > a.md-nav__link, :scope > .md-nav > .md-nav__list > .md-nav__item > a.md-nav__link",
          )
          .forEach(function (childLink) {
            try {
              var childPath = normalizeNavPath(
                new URL(childLink.href, window.location.origin).pathname,
              );
  ...
  function ensureMobileWorkNavLink() {
    document
      .querySelectorAll(".sg-injected-work-link")
      .forEach(function (node) {
        node.remove();
      });
    ...
    primaryList
      ...
      .forEach(function (item) {
        ...
        var sectionLink = item.querySelector(
          ":scope > .md-nav__container > a.md-nav__link, :scope > a.md-nav__link",
        );
        if (!sectionLink) {
          return;
        ...
        try {
          var sectionPath = normalizeNavPath(
            new URL(sectionLink.href, window.location.href).pathname,
          );
          var sectionLabelNode = sectionLink.querySelector(".md-ellipsis");
          var sectionLabel = sectionLabelNode
            ? (sectionLabelNode.textContent || "").trim()
            : (sectionLink.textContent || "").trim();
          if (
            sectionPath === "/web-platforms/" ||
            sectionLabel === "Work" ||
            sectionLabel === "دراسات الحالة"
          ) {
            workSection = item;
          }
        } catch (e) {}
    ...
    var sectionLink = workSection.querySelector(
      ":scope > .md-nav__container > a.md-nav__link, :scope > a.md-nav__link",
    );
    ...
    subList
      .querySelectorAll(":scope > .md-nav__item > a.md-nav__link")
      .forEach(function (link) {
        if (hasWorkLink) {
          return;
        }
        try {
          ...
          var childLabel = childLabelNode
            ? (childLabelNode.textContent || "").trim()
            : (link.textContent || "").trim();
          if (
            childPath === "/web-platforms/" ||
            childLabel === "Work" ||
            childLabel === "دراسات الحالة"
          ) {
            hasWorkLink = true;
          }
        } catch (e) {}
    ...
    var label = labelNode
      ? (labelNode.textContent || "").trim()
      : (sectionLink.textContent || "").trim() || "Work";
  ...
  function ensureMobileProductsNavLink() {
    document
      .querySelectorAll(".sg-injected-products-link")
      .forEach(function (node) {
        node.remove();
      });
    ...
    var subList = productsSection.querySelector(
      ":scope > .md-nav > .md-nav__list",
    );
    if (!subList) {
      return;
    }
    ...
    link.className = "md-nav__link";
    link.href =
      normalizeNavPath(window.location.pathname).indexOf("/ar/") === 0
        ? "/ar/products/"
        : "/products/";
  ...
  function localizeHeaderUI() {
    ...
    document
      .querySelectorAll(".md-tabs__dropdown-link")
      .forEach(function (link) {
        try {
    ...
    document
      .querySelectorAll(".md-sidebar--primary a.md-nav__link")
      .forEach(function (link) {
    ...
    document
      .querySelectorAll(
        ".md-sidebar--primary .md-nav--primary > .md-nav__title, .md-sidebar--primary .md-nav--primary > label.md-nav__title",
      )
      .forEach(function (el) {
    ...
    document
      .querySelectorAll(".md-sidebar--secondary .md-nav__title")
      .forEach(function (el) {
  ...
  function applyReveal() {
    var items = document.querySelectorAll(
      ".md-typeset .grid.cards > ul > li, .card-grid .info-card, .terminal-hero, .page-hero",
    );
    if (!items.length) {
      return;
    }
  ...
  function enhancePageHero() {
    ...
    if (
      !typeset ||
      typeset.querySelector(".terminal-hero") ||
      typeset.querySelector(".page-hero") ||
      document.querySelector(".bp-ph")
    ) {
      return;
    }
    ...
    var pathBits = window.location.pathname
      .replace(/\/+$/, "")
      .split("/")
      .filter(Boolean);
